    ...
    <div class="box">
      ...
      <form action="login.php" method="post">
        ...
        if (isset($_POST['login'])) {
          ...
          $sql1 = "select * from customer where username='$username' and password='$pass'";
          ...
          if (mysqli_num_rows($ad) == 1) {
            ...
            header("Location:Admin/cusrecord.php");

          } else if (mysqli_num_rows($cus) > 0) {
            $select = mysqli_query($con, $sql1);
            while ($fetch_cus = mysqli_fetch_assoc($select)) {
              $_SESSION['user_id'] = $fetch_cus['cust_id'];
            }
            $_SESSION['username'] = $useranme;
            $_SESSION['customer_login_status'] = "loged in";
            header("Location:customer/transaction.php");
          } else {
